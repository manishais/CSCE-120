...
int main() {
  ...
  int targetWidth = 0;
  int targetHeight = 0;
  int* seamToRemove = new int [height] ;
  ...
  {
    cout << "target width must be less than width, " << targetWidth << " is greater than " << width << endl ;
    return 0 ;
  }

   if(targetHeight > height)
  {
    cout << "target height must be less than height, " << targetHeight << " is greater than " << height << endl ;
    return 0 ;
  }
  ...
  if (image != nullptr) {
    if (loadImage(filename, image, width, height)) {
      ...
      {
        seamToRemove = findMinVerticalSeam(image, width, height) ;
        removeVerticalSeam(image, width, height, seamToRemove) ;
